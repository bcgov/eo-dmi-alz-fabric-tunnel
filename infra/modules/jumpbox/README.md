...
- **Entra ID SSH Login**: Microsoft Entra ID (AAD) authentication via `AADSSHLoginForLinux` VM extension; no SSH key pair is generated
...
There is no SSH private key output or local key file. Interactive access is expected to use Entra ID through Azure Bastion.
...
- **Entra ID SSH Authentication**: Password authentication disabled and no SSH key pair generated by Terraform
